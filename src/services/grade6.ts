  ...
  switch (topic.id) {
    case 'g6_fractions_addition': {
        if (difficulty === '基礎') {
            ...
            const n2 = randInt(1, d2 - 1);
            text = `${whole1}と${n1}/${d1} + ${whole2}と${n2}/${d2} = ? (仮分数で答えなさい)`;
    ...
    case 'g6_fractions_subtraction': {
        ...
        text = `${bigW ? `${bigW}と` : ''}${bigN}/${bigD} - ${smallW ? `${smallW}と` : ''}${smallN}/${smallD} = ? (仮分数で答えなさい)`;
    ...
    case 'g6_distance': {
        ...
        if (difficulty === '基礎') {
            ...
            if (questionType === 'g6_speed') {
                ...
                text = `時速${speedInKmh}kmで${distInM}m進むには何分何秒かかりますか？(秒未満は切り捨て。例: 5,30)`;
                const timeInSec = Math.floor((distInM / 1000) / speedInKmh * 3600);
                answer = `${Math.floor(timeInSec / 60)},${timeInSec % 60}`;
                explanation = `時間 = 距離 ÷ 速さ。単位をmと秒に揃えて計算し、最後に分秒に直します。答えは${Math.floor(timeInSec / 60)}分${timeInSec % 60}秒です。`;
            } else { // distance
    ...
    case 'g6_ratios': {
        ...
        if (difficulty === '基礎') {
            ...
            do {
                a = randInt(1, 5);
                b = randInt(1, 5, [a]);
            } while (gcd(a, b) !== 1); // Ensure a and b are coprime

            text = `${a * common}:${b * common} の比を最も簡単な整数の比にしなさい。(例: 2:3)`;
            answer = `${a}:${b}`;
            ...
            if (Math.random() < 0.5) { // decimals
                ...
                text = `${a/10}:${b/10} の比を最も簡単な整数の比にしなさい。(例: 2:3)`;
                ...
                const n1 = randInt(1, d1-1);
                const n2 = randInt(1, d2-1);
                text = `${n1}/${d1}:${n2}/${d2} の比を最も簡単な整数の比にしなさい。(例: 2:3)`;
                const commonDen = lcm(d1, d2);
            ...
            const gcd3 = (n1: number, n2: number, n3: number): number => gcd(gcd(n1, n2), n3);
            do {
              a = randInt(1, 4);
              b = randInt(1, 4, [a]);
              c = randInt(1, 4, [a, b]);
            ...
            text = `${a * common}:${b * common}:${c * common} の比を最も簡単な整数の比にしなさい。(例: 2:3:4)`;
            answer = `${a}:${b}:${c}`;
