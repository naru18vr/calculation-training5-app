  ...
  switch (topic.id) {
    ...
    case 'm1_algebra_distributive': { // 分配法則
        if (difficulty === '基礎') {
            ...
            explanation = `基礎：分配法則を使ってかっこを外します。${a}をかっこの中の各項に掛けます。答えは ${answer}。`;
        } else if (difficulty === '標準') {
            const a = randInt(2, 5);
            const b = randInt(1, 5);
            const c = randInt(1, 5);
            ...
            if(res === '') res = '0';
            answer = res;
            explanation = `標準：分配法則でかっこを外し、同類項をまとめます。答えは ${answer}。`;
        } else { // 発展
            const a = randInt(2, 5);
            const b = randInt(1, 5);
            const c = randInt(1, 5);
            ...
            if(res === '') res = '0';
            answer = res;
            explanation = `発展：分配法則でかっこを外し、同類項をまとめます。-d(...)のかっこを外すときは符号が変わることに注意。答えは ${answer}。`;
        }
        break;
    }
    case 'm1_linear_equations_basic': { // 一次方程式 基本
        ...
        if (difficulty === '基礎') {
            if (Math.random() < 0.5) {
                const a = randInt(1, 10);
                text = `x + ${a} = ${x + a} を解きなさい。(例: x=5)`;
                answer = `x=${x}`;
                explanation = `基礎：両辺から${a}を引くと x = ${x} となります。`;
            } else {
                const a = randInt(2, 5);
                text = `${a}x = ${a * x} を解きなさい。(例: x=5)`;
                answer = `x=${x}`;
            ...
            const c = a * x + b;
            text = `${a}x ${b > 0 ? `+ ${b}` : `- ${-b}`} = ${c} を解きなさい。(例: x=5)`;
            answer = `x=${x}`;
    ...
    case 'm1_linear_equations_parentheses': { // 一次方程式 かっこ
        ...
        if (difficulty === '基礎') {
            ...
            const e = a * (x + b) - c * (x + d);
            text = `${a}(x ${b>0?`+ ${b}`:`- ${-b}`}) - ${c}(x ${d>0?`+ ${d}`:`- ${-d}`}) = ${e} を解きなさい。(例: x=5)`;
            answer = `x=${x}`;
            ...
            const b = randInt(-5, 5, [0]);
            const c = (x+b)/d;
            text = `(x ${b>0?`+ ${b}`:`- ${-b}`}) / ${d} = ${c} を解きなさい。(例: x=5)`;
            answer = `x=${x}`;
    ...
    case 'm1_direct_proportion': { // 比例
        ...
        if (difficulty === '基礎') {
            ...
            const x = randInt(2, 5);
            const y = a * x;
            text = `yはxに比例し、x=${x}のときy=${y}です。yをxの式で表しなさい。(例: y=2x)`;
    ...
    case 'm1_inverse_proportion': { // 反比例
        ...
        if (difficulty === '基礎') {
            ...
            const x = [2, 3, 4, 6].find(n => k%n === 0) || 2;
            const y = k / x;
            text = `yはxに反比例し、x=${x}のときy=${y}です。yをxの式で表しなさい。(例: y=12/x)`;
            answer = `y=${k}/x`;
